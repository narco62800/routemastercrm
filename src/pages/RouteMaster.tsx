...
export default function RouteMaster() {
  ...
  const [subjectNames, setSubjectNames] = useState<Record<string, string>>(() => {
    const saved = localStorage.getItem('routemaster_subjects_v5');
  ...
  // Liaison directe avec le fichier PDF déposé sur GitHub
  const [chapterDocs, setChapterDocs] = useState<Record<string, { docUrl?: string; isVisible: boolean }>>(() => {
    return {
      'Terminales CRM__cours__Bilan première': { docUrl: '/bilan des acquis.pdf', isVisible: true },
  ...
  // Actions Chapitres & Liseuse PDF
  const [pdfViewer, setPdfViewer] = useState<{ title: string; url: string } | null>(null);
  const [renamingTitle, setRenamingTitle] = useState<string | null>(null);
  const [renameValue, setRenameValue] = useState('');

  // OUVERTURE DIRECTE ET SANS ERREUR DU PDF
  const handleOpenDoc = (c: Chapter) => {
    let url = '/bilan des acquis.pdf';
    
    const key = getChapterKey(c);
    if (chapterDocs[key]?.docUrl) {
      url = chapterDocs[key].docUrl!;
    }

    setPdfViewer({ 
      title: c.title, 
      url: encodeURI(url) + '#toolbar=0&navpanes=0&scrollbar=0' 
    });
  };
  ...
  const handleAddChapter = () => {
    ...
    setChapterDocs(prev => ({
      ...prev,
      [key]: { docUrl: '/bilan des acquis.pdf', isVisible: true }
    }));
